...
const Information = ({ planet, planetData, selectedPlanetData, sunData }) => {

	// const [selectedPlanetData, setSelectedPlanetData] = useState();

    // useEffect(() => {
    //     setSelectedPlanetData(planetData.filter((planetEle => planetEle.name === planet)))
    // }, [planet])
	console.log(sunData);

	const displayData = selectedPlanetData === "Sun" ? sunData : selectedPlanetData;
	...
	return (
		<div className="planet-information-container">
			...
			<div className="planet-text-container">{text}</div>
		</div>
	);
};
